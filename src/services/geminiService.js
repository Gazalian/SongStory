...
5. **NIGERIAN FOCUS**: When searching for Nigerian artists (Asake, Wizkid, Burna Boy, Davido, etc.), prioritize accurate local information.`;
     ...
     */
    getModel(model, systemInstruction) {
        const params = {
     ...
     */
    async tryWithModels(prompt, customSystemInstruction) {
     ...
     */
    async generateText({ prompt, model, systemInstruction, }) {
        try {
     ...
     */
    async generateWithImages({ prompt, images, model, systemInstruction, }) {
        try {
            const genModel = this.getModel(model, systemInstruction);
            const parts = [
                { text: prompt },
     ...
     */
    async sendChatMessage({ sessionId, message, model, systemInstruction, }) {
        try {
    ...
    Return a JSON object with this exact structure:
    {
      ...
      "mood": "emotional tone/vibe"
            ...
            if (data) {
                ...
                if (!data.imageUrl)
                    data.imageUrl = this.getDefaultImage('song');
                if (!data.themeColor)
     ...
     */
    async getAlbumStory(title, artist) {
        try {
    ...
    Return a JSON object with this exact structure:
    {
      ...
      "hiddenDetails": "interesting behind-the-scenes facts"
            ...
            if (data) {
                ...
                data.sources = this.extractSources({});
                if (!data.imageUrl)
                    data.imageUrl = this.getDefaultImage('album');
                if (!data.themeColor)
                    data.themeColor = this.getDefaultColor('album');
                if (!data.tracklist)
                    data.tracklist = [];
                if (!data.snapshot)
                    data.snapshot = {
    ...
    Return a JSON object with this exact structure:
    {
      ...
      "visualStyle": "description of visual aesthetic"
            ...
            if (data) {
                ...
                data.sources = this.extractSources({});
                if (!data.imageUrl)
                    data.imageUrl = this.getDefaultImage('artist');
                if (!data.themeColor)
                    data.themeColor = this.getDefaultColor('artist');
                if (!data.careerJourney)
                    data.careerJourney = [];
                if (!data.discographyHighlights)
                    data.discographyHighlights = [];
                if (!data.quotes)
                    data.quotes = [];
                if (!data.trivia)
                    data.trivia = [];
     ...
     */
    async logAvailableModels(apiKey) {
        try {
...
export const getSongStory = async (title, artist) => {
    return await geminiService.getSongStory(title, artist);
};
export const getArtistStory = async (name) => {
    return await geminiService.getArtistStory(name);
};
export const getAlbumStory = async (title, artist) => {
