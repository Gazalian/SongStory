...
async function fetchFromMusixmatch(title, artist, apiKey) {
    try {
        // Search for the track
        const searchUrl = `/api/musixmatch/track.search` +
        ...
        const lyricsUrl = `/api/musixmatch/track.lyrics.get` +
            `?track_id=${trackId}` +
            `&apikey=${apiKey}`;
...
export async function fetchLyrics(title, artist) {
    const musixmatchKey = import.meta.env.VITE_MUSIXMATCH_API_KEY;
    if (musixmatchKey) {
