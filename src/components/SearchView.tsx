...
const FloatingIcon: React.FC<{
  ...
  <div
    className={`absolute text-white/5 pointer-events-none ${className}`}
    style={style}
  >
    <Icon />
  </div>
    ...
    <div className="flex-1 flex flex-col relative w-full h-full overflow-hidden min-h-screen">
      ...
      <div
        className={`absolute inset-0 transition-opacity duration-1000 ${
          hasResults ? "opacity-20" : "opacity-100"
        }`}
      >
        ...
        <FloatingIcon
          Icon={Guitar}
          className="top-[15%] left-[10%] w-24 h-24 animate-drift"
          style={{ animationDuration: "25s" }}
        />
        <FloatingIcon
          Icon={Piano}
          className="top-[25%] right-[15%] w-32 h-32 animate-drift-reverse"
          style={{ animationDuration: "30s" }}
        />
        <FloatingIcon
          Icon={Disc}
          className="bottom-[30%] left-[20%] w-20 h-20 animate-spin-slow opacity-10"
        />
        <FloatingIcon
          Icon={Mic2}
          className="top-[40%] right-[30%] w-16 h-16 animate-float"
        />
        <FloatingIcon
          Icon={Drum}
          className="bottom-[20%] right-[10%] w-24 h-24 animate-drift"
          style={{ animationDuration: "28s" }}
        />
        <FloatingIcon
          Icon={Radio}
          className="top-[10%] left-[40%] w-12 h-12 animate-float-fast"
        />
      ...
      {/* Main Container */}
      <div
        className={`relative z-20 flex-1 flex flex-col transition-all duration-700 w-full max-w-5xl mx-auto px-4 sm:px-6 ${
          hasResults ? "pt-6 pb-20" : "justify-center items-center pb-32"
        }`}
      >
        {/* Title */}
        <div
        ...
        >
          <h1 className="font-display text-5xl md:text-8xl font-black tracking-tighter mb-6 flex flex-col md:flex-row items-center justify-center gap-2 md:gap-6">
            <span className="text-transparent bg-clip-text bg-gradient-to-br from-white via-indigo-200 to-slate-400 drop-shadow-2xl animate-pulse-slow">
              SongStory
            </span>
          </h1>
          <p className="text-zinc-400 font-sans text-lg md:text-xl max-w-lg mx-auto leading-relaxed">
            Uncover the deep cuts, the hidden meanings, and the stories that
            made the music.
          </p>
        </div>
        ...
        {/* Search Bar */}
        <div
          className={`w-full transition-all duration-700 z-30 ${
            hasResults ? "max-w-full" : "max-w-xl"
          }`}
        >
          <form onSubmit={onSearch} className="relative group">
            {/* Glow behind input */}
            <div
              className={`absolute -inset-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-3xl blur opacity-20 group-hover:opacity-50 transition duration-1000 ${
                hasResults ? "opacity-20" : ""
              }`}
            ></div>
            ...
            <div className="relative flex items-center">
              ...
              />
              <Search
                className="absolute left-5 text-zinc-400 group-focus-within:text-white transition-colors"
                size={hasResults ? 20 : 24}
              />
          ...
          <div
            className={`mt-4 text-center transition-opacity duration-300 ${
              hasResults ? "opacity-0 h-0 overflow-hidden" : "opacity-100"
            }`}
          >
        ...
        {hasResults && (
          <div className="mt-8 pb-12 w-full animate-in slide-in-from-bottom-8 duration-700">
            <div
              className={`flex items-center justify-between mb-4 px-1 transition-opacity duration-300 ${
            ...
            >
              ...
              <span className="text-xs font-medium text-zinc-600 bg-white/5 px-2 py-1 rounded-md border border-white/5">
                    ...
                    <div className="flex items-center p-3 sm:p-4 gap-4">
                      ...
                      >
                        {showImage ? (
                          ...
                          <>
                            {result.type === EntityType.Song && (
                              <Music size={24} />
                            )}
                            {result.type === EntityType.Artist && (
                              <Mic2 size={24} />
                            )}
                            {result.type === EntityType.Album && (
                              <Disc size={24} />
                            )}
                          </>
                        )}
            ...
            <div className="flex flex-wrap justify-center gap-3 max-w-lg mx-auto">
              {recentSearches.map((term, index) => (
                <button
                  ...
                  onClick={() => {
                    setQuery(term);
                    onSearch({ preventDefault: () => {} } as any);
                  }}
